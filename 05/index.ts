...
func(8);
func();
// ?는 다음과 동일 -> func(x :number|undefined)
...
*/
function countLength(target :string|number) :number {
    const toStr = target+'';
    return toStr.startsWith('-') ? toStr.length-1 : toStr.length;
}
...
const canUMarry = (income :number, isHomeOwner :boolean, charmfulness :string) :string|void => {
    let score = income;
    if(isHomeOwner) score += 500;
    if(charmfulness === '상') score += 100;
    if(score >= 600) return '결혼가능';
}
console.log(canUMarry(700, false, '중'));
console.log(canUMarry(100, false, '상'));
