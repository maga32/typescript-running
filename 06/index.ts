...
const myFunc1 = (x :string|number) :string|number => {
    // 아래처럼 그냥 더해주면 에러
    // return x + 1;
    if (typeof x === 'string') {
        return x + '1';
...
const myFunc2 = (x :string|number) :void => {
    const array :number[] = [];
    // 얘도 타입검증 없이 넣으면 에러
    if (typeof x === 'number') {
        array[0] = x;
...
const myFunc3 = (x :string|number) :void => {
    const array :number[] = [];
    // x를 넘버로 인정
    array[0] = x as number;
}
...
모르는 부분은 구글검색해도 봐드림 
*/
const cleanToNumber = (arr :(number|string)[]) :number[] => {
...
*/
const getLastSubject = (obj :{subject :(string|string[])}) :string => {
    let result :string = '';
